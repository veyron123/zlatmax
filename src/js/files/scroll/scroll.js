...
export function stickyBlock() {
	addWindowScrollEvent = true;
	// data-sticky для батьківського елементу всередині якого прилипає блок *
	// data-sticky-header для батьківського елементу, враховуємо висоту хедера
	// data-sticky-top="" для батьківського елементу, можна вказати відступ зверху
	// data-sticky-bottom="" для батьківського елементу, можна вказати відступ знизу
	// data-sticky-item для прилипаючого блоку *
			...
			if (!stickyConfig.media || stickyConfig.media < window.innerWidth) {
				if (offsetTop + stickyConfig.bottom + stickyBlockItem.offsetHeight < window.innerHeight) {
					if (scrollY >= startPoint && scrollY <= endPoint) {
						...
						stickyItemValues.left = `${stickyBlockItem.getBoundingClientRect().left}px`; // Врахувати різницю у ширині екрана?
